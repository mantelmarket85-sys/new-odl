...
const FocalActivityLogs = () => {
  const [search, setSearch] = useState("");
  const [actorFilter, setActorFilter] = useState("all");
  const [entityFilter, setEntityFilter] = useState("all");

  const { data, loading, error, reload } = useApi(() => api.focal.audit("?pageSize=200"), []);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Activity Logs"
        subtitle="Detailed audit trail of system actions performed across the department."
        icon="ScrollText"
        breadcrumb={["Focal Person", "Activity Logs"]}
        actions={
...
export default FocalActivityLogs;
