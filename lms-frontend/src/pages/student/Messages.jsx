...
import { MessageSquare, Send, Loader2, Search, Paperclip, FileText, X, Hash, Users, CheckCheck, Check } from "lucide-react";
...
const Messages = () => {
  ...
  const { data, loading, error, reload } = useApi(() => api.student.messageContacts(), []);
  ...
  useEffect(() => {
    // Students only use course groups — auto-open the first group.
    if (!active && groups.length > 0) setActive({ kind: "group", ...groups[0] });
  }, [groups, active]);
  ...
  const loadConversation = useCallback((a) => {
    ...
    setLoadingConv(true);
    setConvError("");
    const p = a.kind === "group" ? api.student.groupConversation(a.id) : api.student.conversation(a.id);
    p
      .then((res) => {
  ...
  useEffect(() => { if (active) loadConversation(active); /* eslint-disable-next-line */ }, [active?.kind, active?.id]);

  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages]);

  useRealtime(api.student.eventsUrl, {
    message: (d) => {
      const a = activeRef.current;
      if (!d) return;
      if (d.action === "direct") {
        if (a && a.kind === "direct" && (String(d.from) === String(a.id) || String(d.to) === String(a.id))) {
          setMessages((m) => (m.some((x) => x.id === d.message.id) ? m : [...m, { ...d.message, mine: String(d.from) !== String(a.id) }]));
        } else { reload(); }
  ...
  const pickFile = async (file) => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await api.student.uploadMessageAttachment(fd);
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = active.kind === "group"
        ? await api.student.sendGroupMessage(active.id, payload)
        : await api.student.sendMessage(active.id, payload);
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter((c) => !q || c.name.toLowerCase().includes(q) || (c.courses || []).join(" ").toLowerCase().includes(q));
  }, [contacts, search]);
  ...
  if (loading) {
    return (
      <div>
        <PageHeader title="Messages" subtitle="Chat with teachers and course groups" icon="MessageSquare" breadcrumb={["Dashboard", "Messages"]} />
  ...
  return (
    <div>
      <PageHeader title="Messages" subtitle="Communicate through your course groups" icon="MessageSquare" breadcrumb={["Dashboard", "Messages"]} />

      {error ? (
        <ErrorState description={error} onRetry={reload} />
      ) : groups.length === 0 ? (
        <EmptyState icon="MessageSquare" title="No course groups yet" description="A chat group is automatically created for every course you enroll in, including your teacher and classmates. Register for courses to join their group." />
      ) : (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col">
            <div className="p-3 border-b border-slate-100 dark:border-slate-800">
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search course groups..." className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm bg-slate-50 dark:bg-slate-800" />
              </div>
            </div>
            <div className="overflow-y-auto flex-1">
              ...
              {filteredGroups.length === 0 ? (
                <p className="px-4 py-6 text-xs text-slate-400 text-center">No course groups found.</p>
              ) : filteredGroups.map((g) => (
                <button key={`g${g.id}`} onClick={() => setActive({ kind: "group", ...g })} className={`w-full flex items-center gap-3 px-4 py-3 text-left border-b border-slate-50 dark:border-slate-800/60 ${active?.kind === "group" && active?.id === g.id ? "bg-primary-50 dark:bg-primary-900/20" : "hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}>
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold shrink-0"><Hash size={18} /></div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{g.memberCount} members{g.courseCode ? ` · ${g.courseCode}` : ""}</p>
                  </div>
              ...
              <>
                <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
                  {active.kind === "group" ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary-600 to-blue-500 flex items-center justify-center text-white font-bold">{initials(active.name)}</div>
                  )}
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                      {active.kind === "group" ? `${active.memberCount} members` : <span className={isOnline ? "text-emerald-500 font-medium" : ""}>{isOnline ? "● Online" : "Offline"}</span>}
                    </p>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50 dark:bg-slate-950/30">
                  {loadingConv ? (
                    ...
                    <div className="h-full flex items-center justify-center text-sm text-slate-400">No messages yet. Say hello!</div>
                  ) : (
                    messages.map((m) => (
                      <motion.div key={m.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`flex ${m.mine ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[75%] px-3.5 py-2 rounded-2xl ${m.mine ? "bg-primary-600 text-white rounded-br-sm" : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-bl-sm border border-slate-100 dark:border-slate-700"}`}>
                ...
                <div className="p-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
                  />
                  <button onClick={send} disabled={sending || (!draft.trim() && !attachment)} className="p-2.5 rounded-xl bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50">
                    {sending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                  </button>
                </div>
              </>
            ) : (
              <div className="h-full flex items-center justify-center text-sm text-slate-400"><MessageSquare size={32} className="mr-2 opacity-40" /> Select a conversation</div>
            )}
          </div>
        </div>
      )}
